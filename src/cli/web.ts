...
import { esc } from "../web/markdown.js";
import {
  type SourceKind,
  type EntryTier,
  type EntryClass,
...
import { handleRequest, type RouterContext } from "../web/router.js";
...
export async function run(args: string[]) {
  ...
  const server = Bun.serve({
    ...
    fetch: async (request) => {
      ...
      // Delegate all API requests to router
      if (url.pathname.startsWith("/api/")) {
        return handleRequest(request, routerContext);
      }
